...
if [[ "$root_fs" != "btrfs" ]]; then
  printf '%s\n' "FAIL: expected btrfs root for rollback-capable install" >&2
  exit 1
fi
...
if systemctl is-active snapper-timeline.timer | grep -qx 'active'; then
  printf '%s\n' "PASS: snapper-timeline.timer active"
else
  printf '%s\n' "FAIL: snapper-timeline.timer inactive" >&2
  exit 1
fi

if systemctl is-active snapper-cleanup.timer | grep -qx 'active'; then
  printf '%s\n' "PASS: snapper-cleanup.timer active"
else
  printf '%s\n' "FAIL: snapper-cleanup.timer inactive" >&2
  exit 1
fi
