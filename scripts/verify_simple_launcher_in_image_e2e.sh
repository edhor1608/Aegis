...
echo "$DRY_OUTPUT" | grep -q "SENIOR_ZERO_LAUNCHER_GUI"
...
RUN_OUTPUT="$(SENIOR_ZERO_LAUNCHER_NO_EXEC=1 SENIOR_ZERO_LAUNCHER_NO_GUI=1 bash "$SCRIPT" --action help)"
echo "$RUN_OUTPUT" | grep -q "SENIOR_ZERO_LAUNCHER_ACTION_EXECUTED"

RUN_ACCEPTANCE_OUTPUT="$(SENIOR_ZERO_LAUNCHER_NO_EXEC=1 SENIOR_ZERO_LAUNCHER_NO_GUI=1 bash "$SCRIPT" --action acceptance)"
echo "$RUN_ACCEPTANCE_OUTPUT" | grep -q "action=acceptance command=senior-zero-acceptance-runner --dry-run"

GUI_OUTPUT="$(SENIOR_ZERO_LAUNCHER_NO_EXEC=1 SENIOR_ZERO_LAUNCHER_NO_GUI=1 bash "$SCRIPT" --gui)"
echo "$GUI_OUTPUT" | grep -q "SENIOR_ZERO_LAUNCHER_GUI"
...
grep -q "Exec=senior-zero-launcher --gui" "$DESKTOP"
